...
var pattern = require('./config/pattern').pattern;
...
var cleanBuilds = function cleanBuilds() {
  var deleteDirectories = _shelljs2.default.rm('-rf', [
    _path2.default.join(__dirname, 'ios/build/*'),
    _path2.default.join(__dirname, 'android/.gradle/*'),
    _path2.default.join(__dirname, 'android/app/build/*'),
    _path2.default.join(__dirname, 'android/build/*'),
  ]);
  Promise.resolve(deleteDirectories);
  console.log('Done removing builds.'.green);
};

readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml'))
  .then(function (data) {
    ...
    _commander2.default
      ...
      .action(function (newName) {
        ...
        var listOfFilesToModifyContent = (0, _filesToModifyContent.filesToModifyContent)(
          currentAppName,
          newName,
          projectName
        );
        ...
        var resolveFoldersAndFiles = new Promise(function (resolve) {
          listOfFoldersAndFiles.forEach(function (element, index) {
            ...
            setTimeout(function () {
              ...
              if (
                _fs2.default.existsSync(_path2.default.join(__dirname, element)) ||
                !_fs2.default.existsSync(_path2.default.join(__dirname, element))
              ) {
                var move = _shelljs2.default.exec(
                  'git mv "' +
                    _path2.default.join(__dirname, element) +
                    '" "' +
                    _path2.default.join(__dirname, dest) +
                    '" 2>/dev/null'
                );
        ...
        var resolveJavaFiles = function resolveJavaFiles() {
          return new Promise(function (resolve) {
            readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
              ...
              var fullCurrentDebugBundlePath = _path2.default.join(__dirname, currentJavaPath.replace('main', 'debug'));
              var fullNewDebugBundlePath = _path2.default.join(__dirname, newBundlePath.replace('main', 'debug'));
              ...
              if (!_fs2.default.existsSync(fullNewBundlePath)) {
                ...
                var move = _shelljs2.default.exec(
                  'git mv "' + fullCurrentBundlePath + '/"* "' + fullNewBundlePath + '" 2>/dev/null'
                );
                ...
                move = _shelljs2.default.exec(
                  'git mv "' + fullCurrentDebugBundlePath + '" "' + fullNewDebugBundlePath + '" 2>/dev/null'
                );
                successMsg =
                  newBundlePath.replace('main', 'debug') + ' ' + _colors2.default.green('BUNDLE INDENTIFIER CHANGED');
                if (move.code === 0) {
                  console.log(successMsg);
                } else if (move.code === 128) {
                  // if "outside repository" error occured
        ...
        var resolveBundleIdentifiers = function resolveBundleIdentifiers(params) {
          return new Promise(function (resolve) {
            ...
            (0, _bundleIdentifiers.bundleIdentifiers)(
              currentAppName,
              newName,
              projectName,
              currentBundleID,
          ...
          resolveFoldersAndFiles
            ...
            .then(function () {
              if (_fs2.default.existsSync(_path2.default.join(__dirname, 'ios', 'Podfile'))) {
                console.log(
                  '' +
                    _colors2.default.yellow('Podfile has been modified, please run "pod install" inside ios directory.')
                );
              }
            })
            .then(function () {
